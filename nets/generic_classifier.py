...
class GenericClassifier(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.train_step_outputs.append({"key": batch[1], "y": y.cpu(), "scores": scores.detach().cpu(), "loss": loss.detach().cpu(), "preds": preds.cpu()})
        return loss

    def validation_step(self, batch, batch_idx):
        loss, y, scores, _, preds = self._compute_loss_and_acc(batch, mode=True)
        self.validation_step_outputs.append({"key": batch[1], "y": y.cpu(), "scores": scores.detach().cpu(), "loss": loss.detach().cpu(), "preds": preds.cpu()})
        return loss

    def test_step(self, batch, batch_idx):
        loss, y, scores, _, preds = self._compute_loss_and_acc(batch, mode=False)
        self.test_step_outputs.append({"key": batch[1], "y": y.cpu(), "scores": scores.detach().cpu(), "loss": loss.detach().cpu(), "preds": preds.cpu()})
        return loss
    ...
    def _aggregate_step_outputs(self, outputs, stage="test"):
        ...
        preds_summary = ["bonafide" if label == 1 else "fake" for label in preds]

        log, th = eval.compute_eer_ori(y, y_hat, preds)
    ...
    def on_train_epoch_end(self):
        ...
        train_eer, train_acc = self._aggregate_step_outputs(self.train_step_outputs, "train")
    ...
    def on_validation_epoch_end(self):
        ...
        val_eer, val_acc = self._aggregate_step_outputs(self.validation_step_outputs, "val")
    ...
    def on_test_end(self):
        ...
        avg_loss = np.mean(all_loss)
        self.logger.experiment.add_scalar("test_loss", avg_loss)
        print("\033[92m####### EVALUATION #######\033[0m")
        test_eer, _ = self._aggregate_step_outputs(self.test_step_outputs, "test")
        self.logger.experiment.add_scalar("test_eer", test_eer)
