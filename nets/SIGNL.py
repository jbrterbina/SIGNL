...
class GCN(nn.Module):
    def __init__(self, hidden_dim, num_layers, num_heads=4, p=0.):
        ...
        for i in range(num_layers):
            ...
            self.conv1d.append(nn.Conv1d(out_features, out_features, 1, bias=True, groups=out_features // num_heads))
...
class Encoder(nn.Module):
    def __init__(self, p=0.1, num_layers=4, k=2, num_channel=16, spec_size=(20, 20), drop_edge=False, gaussian_noise=False, node_feature_masking=False):
    ...
    def forward(self, x):
        ...
        edge_index = knn_graph(x.float(), k=self.k, batch=batch, loop=False, cosine=True)
        if self.drop_edge:
...
class GCS_enc_trn(pl.LightningModule):
    def __init__(self, config, sweep=False):
        ...
        self.encoder_t = Encoder(num_channel=num_channel, num_layers=num_layers, k=k, spec_size=(num_node, num_node), drop_edge=config["model"]["de"], gaussian_noise=config["model"]["gn"], node_feature_masking=config["model"]["fm"])
        self.encoder_f = Encoder(num_channel=num_channel, num_layers=num_layers, k=k, spec_size=(num_node, num_node), drop_edge=config["model"]["de"], gaussian_noise=config["model"]["gn"], node_feature_masking=config["model"]["fm"])
        ...
        if config["model"]["spec"] == "raw":
            self.feature = WaveformCNN()
        else:
            self.feature = Spectrogram(
                config["model"]["spec"],
                config["model"]["emphasis"],
                config["model"]["band_trimming"],
                config["model"]["normalise"],
                config["model"]["specaugment"],
                config["model"]["filter_type"],
                config["model"]["log_transform"],
                config["model"]["noise"]
            )
        self.sweep = sweep
    ...
    def forward(self, x):
        ...
        x1_f = F.dropout(self.encoder_f(rearrange(x1, 'b c t f -> b c f t')), p=self.p1, training=self.training)
        x1 = torch.cat((x1_t, x1_f), dim=1)
        x1_g = self.g(x1)
        x2_t = F.dropout(self.encoder_t(x2), p=self.p1, training=self.training)
        x2_f = F.dropout(self.encoder_f(rearrange(x2, 'b c t f -> b c f t')), p=self.p1, training=self.training)
    ...
    def training_step(self, batch):
        ...
        self.train_step_outputs.append({"loss": loss.detach().cpu(), "enc_sim": enc_sim.detach().cpu()})
        return loss
    ...
    def validation_step(self, batch):
        ...
        enc_sim = self.alignment_loss(x1, x2)
        loss = self.alignment_loss(x1_g, x2_g)
        self.validation_step_outputs.append({"loss": loss.detach().cpu(), "enc_sim": enc_sim.detach().cpu()})
    ...
    def test_step(self, batch):
        ...
        enc_sim = self.alignment_loss(x1, x2)
        loss = self.alignment_loss(x1_g, x2_g)
        self.test_step_outputs.append({"loss": loss.detach().cpu(), "enc_sim": enc_sim.detach().cpu()})
    ...
    def on_test_end(self):
        ...
        avg_loss = np.mean(all_loss)
        enc_sim = np.mean(enc_sim)
        self.logger.experiment.add_scalar("test_alignment_loss", avg_loss)
        if self.sweep:
...
class Stem(nn.Module):
    def __init__(self, index_factor=4, in_dim=1, out_dim=16):
        ...
        self.convs = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Conv2d(out_dim // 8, out_dim // 4, 3, stride=(spatial_factors[0], temporal_factors[0]), padding=1),
            nn.BatchNorm2d(out_dim // 4),
            nn.ReLU(),
            nn.Conv2d(out_dim // 4, out_dim // 2, 3, stride=1, padding=1),
            nn.BatchNorm2d(out_dim // 2),
            nn.ReLU(),
            nn.Conv2d(out_dim // 2, out_dim, 3, stride=(spatial_factors[1], temporal_factors[1]), padding=1),
            nn.BatchNorm2d(out_dim),
            nn.ReLU(),
            nn.Conv2d(out_dim, out_dim, 3, stride=(spatial_factors[2], temporal_factors[2]), padding=1),
            nn.BatchNorm2d(out_dim)
        )
...
class GCS_cls(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.p1 = config["model"]["dropout"]
        self.dropout_layer = torch.nn.Dropout(p=self.p1)
        num_channel = 32
        possible_node = [4, 6, 8, 12, 24]
        num_node = possible_node[config["model"]["index_factor"]]
        num_layers = 5
        k = config["model"]["num_k"]
        hidden_dim = num_channel * num_node
        flat_dim = (hidden_dim // (2 ** num_layers)) * num_node
        self.encoder_t = Encoder(num_channel=num_channel, num_layers=num_layers, k=k, spec_size=(num_node, num_node))
        self.encoder_f = Encoder(num_channel=num_channel, num_layers=num_layers, k=k, spec_size=(num_node, num_node))
        self.stem = Stem(index_factor=config["model"]["index_factor"], out_dim=num_channel)
